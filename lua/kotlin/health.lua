...
local function check_install()
    ...
    local resolved
    if mason_exists then
        resolved = kotlin.resolve_kotlin_lsp_dir(mason_root)
    end
    if not resolved and env_dir then
        resolved = kotlin.resolve_kotlin_lsp_dir(env_dir) or env_dir
    end
    ...
    local intellij_server = resolved .. "/bin/intellij-server"
...
local function check_jre()
    ...
    if vim.env.JAVA_HOME then
        probe("$JAVA_HOME java", vim.env.JAVA_HOME .. "/bin/java", true)
    else
...
local function check_clients()
    ...
    for _, c in ipairs(clients) do
        ...
        if type(provider) == "table" and provider.commands then
            ...
            local needed =
                { "exportWorkspace", "kotlin.organize.imports", "interpolateFileTemplate", "start_debug_server" }
